...
grain = 10000;
if isnan(params), sixDates = NaN*ones(1,6); return; end
...
%% Back to analysis for generalized sigmoid and greendown sigmoid
...
if n ~= 7, sixDates = NaN*ones(1,6); end

%% percentile method
%This section overwrites the 1, 2, and 5, 6th dates
%that were estimated with the CCR, using the threshold crossing method.  As
%reported in Klosterman et al 2014.
switch modelName
...
if isnan(sixDates(4)), return; end
...
if (min(sixDates(1:3))<0) || (max(sixDates(1:3))>200)
    sixDates(1:3) = NaN*ones(1,3);
end
if (min(sixDates(4:6))<200) || (max(sixDates(4:6))>365)
    sixDates(4:6) = NaN*ones(1,3);
end